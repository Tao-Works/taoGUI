  ...
  class taoReportReader {
    ...
    public taoReportReader (string taoReportFilename) {
      ...
      if (System.IO.File.Exists(taoReportFilename)) {
        ...
        Form1._taoSheet = (Excel.Worksheet)Form1._taoBook.Sheets[1];
        int totalRows = (int)Form1._taoSheet.Cells.SpecialCells(Excel.XlCellType.xlCellTypeLastCell).Row + 1;
        System.Array _tmpTotalTests = (System.Array)Form1._taoSheet.get_Range("F16", "F" + totalRows.ToString()).Cells.Value;
        List<string> _totalTests = _tmpTotalTests.OfType<string>().ToList();
        _totalTests.RemoveAll(item => item == null);
        System.Array _tmpPairsThatAreEqual = (System.Array)Form1._taoSheet.get_Range("G16", "G" + totalRows.ToString()).Cells.Value;
